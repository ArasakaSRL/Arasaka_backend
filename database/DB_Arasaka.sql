...
CREATE TABLE telefono (
    id_telefono UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    id_usuario UUID NOT NULL,
    telefono VARCHAR(10),
    FOREIGN KEY (id_usuario) REFERENCES usuario (id_usuario) ON DELETE CASCADE
);
...
CREATE TABLE pais (
    id_pais UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    id_usuario UUID NOT NULL,
    nombre VARCHAR(255),
    FOREIGN KEY (id_usuario) REFERENCES usuario (id_usuario) ON DELETE CASCADE
);
...
CREATE TABLE profesion (
    id_profesion UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    nombre VARCHAR(50),
    descripcion VARCHAR(550)
);
...
CREATE TABLE idioma (
    id_idioma UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    nombre VARCHAR(100)
);

CREATE TABLE nivel_ingles (
    id_nivel_ingles UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    id_idioma UUID,
    nivel VARCHAR(2),
...
CREATE TABLE portafolio (
    id_portafolio UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    id_usuario UUID,
    nombre VARCHAR(50),
    visibilidad BOOLEAN,
    descripcion VARCHAR(550),
...
CREATE TABLE configuracion_portafolio (
    id_configuracion_portafolio UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    id_portafolio UUID,
...
CREATE TABLE visualizaciones_portafolio (
    id_visualizacion_portafolio UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    id_portafolio UUID,
    numero INTEGER,
    fecha DATE,
    ip_vista TEXT,
...
CREATE TABLE estado_proyecto (
    id_estado_proyecto UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    id_proyecto UUID,
    estado VARCHAR(255),
...
CREATE TABLE url_imagen_proyecto (
    id_url_imagen_proyecto UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    id_proyecto UUID,
    url_imagen TEXT,
    FOREIGN KEY (id_proyecto) REFERENCES proyecto (id_proyecto)
);
...
CREATE TABLE categoria_habilidad (
    id_categoria_habilidad UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    nombre VARCHAR(100)
);

CREATE TABLE nivel_de_habilidad (
    id_nivel_habilidad UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    nivel VARCHAR(50)
);

CREATE TABLE habilidad (
    id_habilidad UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    id_portafolio UUID,
...
CREATE TABLE tecnologias (
    id_tecnologia UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    id_proyecto UUID,
    id_habilidad UUID,
    nombre VARCHAR(225),
...
CREATE TABLE categoria_tecnologia (
    id_categoria_tecnologia UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    id_tecnologia UUID,
    nombre VARCHAR(225),
...
CREATE TABLE tipo_experiencia (
    id_tipo_experiencia UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    nombre VARCHAR(100)
);

CREATE TABLE experiencia (
    id_experiencia UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    id_portafolio UUID,
...
CREATE TABLE certificacion (
    id_certificacion UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    id_portafolio UUID,
    titulo VARCHAR(100),
    descripcion VARCHAR(550),
...
CREATE TABLE redes_profesionales (
    id_red_profesional UUID DEFAULT gen_random_uuid () PRIMARY KEY,
    id_portafolio UUID,
    nombre VARCHAR(150),
    url TEXT,
    FOREIGN KEY (id_portafolio) REFERENCES portafolio (id_portafolio)
